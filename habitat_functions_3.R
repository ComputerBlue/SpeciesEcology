...
# }
# 
# simulate_passive_dispersal <- function(NB, individual_cells, habitat_cells){
#   
#   new_location_cells <- NULL
#   for (i in individual_cells){
#     sample_neighbour <- sample(NB[[i]],1)
#     in_habitat <- (length(union(habitat_cells, sample_neighbour))>0)
#     if (in_habitat){new_location_cells <- union(new_location_cells, sample_neighbour)}
#   }
#   return(new_location_cells)
# }
# 
# simulate_conspecific_dispersal <- function(NB, individual_cells, habitat_cells, sum_individuals){
#   Nindividuals <- length(individual_cells)
#   new_location_cells <- NULL
#   for (i in seq(1, Nindividuals)){
#     individuals_in_neighbourhood <- sum(sum_individuals[NB[[i]]]) +22
#     sample_neighbour <- sample(NB[[individual_cells[i]]],1, prob = (sum_individuals[NB[[i]]]+1)/individuals_in_neighbourhood)
#     in_habitat <- length(union(habitat_cells, sample_neighbour))>0
#     if (in_habitat){new_location_cells <- union(new_location_cells, sample_neighbour)}
#   }
#   return(new_location_cells)
# }
# 
# simulate_habitat_driven_dispersal <- function(NB, individual_cells, habitat_cells, sum_habitat){
#   Nindividuals <- length(individual_cells)
#   new_location_cells <- NULL
#   for (i in seq(1, Nindividuals)){
#     habitat_in_neighbourhood <- sum(sum_habitat[NB[[i]]]) +22
#     sample_neighbour <- sample(NB[[individual_cells[i]]],1, prob = (sum_habitat[NB[[i]]]+1)/habitat_in_neighbourhood)
...
# }
# 
# simulate_habitat_conspecific_dispersal <- function(NB, individual_cells, habitat_cells, sum_individuals, sum_habitat){
#   Nindividuals <- length(individual_cells)
#   new_location_cells <- NULL
#   for (i in seq(1, Nindividuals)){
#     individuals_in_neighbourhood <- sum(sum_individuals[NB[[i]]]) +22
...
# }
# 
# simulate_habitat_avoidance_driven_dispersal <- function(NB, individual_cells, habitat_cells, sum_individuals, sum_habitat){
#   Nindividuals <- length(individual_cells)
#   new_location_cells <- NULL
#   for (i in seq(1, Nindividuals)){
#     individuals_in_neighbourhood <- sum(sum_individuals[NB[[i]]]) +22
...
#     individuals_in_neighbourhood <- sum(sum_individuals[NB[[i]]]) +22
#     habitat_in_neighbourhood <- sum(sum_habitat[NB[[i]]])
#     sample_neighbour <- sample(NB[[individual_cells[i]]],1, prob = 1/(sum_individuals[NB[[i]]]+1))
#     in_habitat <- (sum_habitat[sample_neighbour]>0)
#     if (in_habitat){new_location_cells <- union(new_location_cells, sample_neighbour)}
#   }
#   return(new_location_cells)
...
# # n_y <- 100
# # 
# # # covariance matrices expensive to create. Save these and generate only once? 
# # l_low <- 1
# # l <- l_low
# # habitat_style <- 'binary'
# # 
# # habitat_ts_lowspatialscale <- generate_habitat_dynamics('binary', 'low-scale', l_low, 'localized', 'decay', n_x, n_y, Kt)
# # ggplot(habitat_ts_lowspatialscale, aes(x =x, y = y, col = habitat))+geom_point()+facet_wrap(.~year, ncol = 5)
# # 
# # l_long <- 35
# # l <- l_long
# # habitat_ts_highspatialscale <- generate_habitat_dynamics('binary', 'long-scale', l_long, 'localized', 'decay', n_x, n_y, Kt)
# # ggplot(habitat_ts_highspatialscale, aes(x =x, y = y, col = habitat))+geom_point()+facet_wrap(.~year, ncol= 5)
# #   
# 
# 
